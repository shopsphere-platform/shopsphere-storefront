...
function Hero() {
    return (
        <section className="relative h-screen flex items-center justify-center overflow-hidden">
            <img
                src="https://images.unsplash.com/photo-1548171915-e79a380a2a4b?w=1800&q=90"
                alt="Hero watch"
                className="absolute inset-0 w-full h-full object-cover"
            />
            <div className="absolute inset-0 bg-black/65" />
            <div className="relative z-10 text-center px-6 max-w-4xl mx-auto">
                <p className="text-xs tracking-[0.4em] uppercase text-yellow-400 mb-6">
                    Est. Geneva · Maison ShopSphere
                </p>
                <h1 className="font-display text-5xl md:text-7xl font-normal text-white mb-6 leading-tight">
                    Time is the only <br />
                    <span className="text-yellow-400 italic">true luxury</span>
                </h1>
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                    <Link to="/collections"
                          className="inline-flex items-center gap-2 bg-yellow-400 text-black px-8 py-3 text-sm font-medium tracking-widest uppercase hover:bg-yellow-300 transition-colors">
                        Explore Collection <ArrowRight size={14} />
                    </Link>
                    <Link to="/about"
                          className="inline-flex items-center gap-2 border border-yellow-400 text-yellow-400 px-8 py-3 text-sm tracking-widest uppercase hover:bg-yellow-400/10 transition-colors">
                        Our Story
                    </Link>
                </div>
            </div>
        ...
        <section className="py-20 px-6">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-white/10">
                    {watches.map(watch => (
                        <Link key={watch.id} to={`/watches/${watch.id}`}
                              className="group bg-black hover:bg-zinc-900 transition-colors">
        ...
        <section className="py-20 px-6 bg-zinc-900 border-t border-white/10">
            <div className="max-w-xl mx-auto text-center">
                ...
                <p className="text-gray-400 text-sm mb-8 leading-relaxed">
                    Private previews, collector events, and notes from our workshops.
                </p>
                <div className="flex gap-0">
                    <input type="email" placeholder="Your email address"
                           className="flex-1 bg-black border border-white/20 text-white text-sm px-5 py-3 outline-none focus:border-yellow-400/50 transition-colors placeholder:text-gray-600" />
                    <button className="bg-yellow-400 text-black text-xs tracking-widest uppercase px-6 font-medium hover:bg-yellow-300 transition-colors">
                        Join
                    </button>
                </div>
            </div>
